...
module overmind::NonFungibleToken {
    ...
    */
    public fun mint_nft(
        ...
        let mint_price = 1000000000;
        assert!(payment_amount >= mint_price,EInsufficientPayment);

        let coin_balance = coin::balance_mut(payment_coin);
        let paid = balance::split(coin_balance,mint_price);

        balance::join(&mut minter_cap.sales,paid);
        
        // transfer::public_transfer(&coin_balance,recipient);

        //let price = coin::split(payment_coin,mint_price,ctx);
        //let price_balance = coin::balance_mut(price);
        //balance::join(&mut minter_cap.sales,price_balance);

        //transfer::transfer(payment_coin,recipient);

        // let name_string=string::from_utf8(nft_name);
        // let description_string =string::from_utf8(nft_description);
        // let url_string =url::new_unsafe_from_bytes(nft_image);
    ...
    */
    public fun combine_nfts(
        ...
        let new_name_vector: vector<u8> = b"";
        let new_name_string: String = string::utf8(new_name_vector);
        string::append(&mut new_name_string, nft1.name);
        let vec1: vector<u8> = b" + ";
        string::append(&mut new_name_string, string::utf8(vec1));
        string::append(&mut new_name_string, nft2.name);

        let new_description_vector: vector<u8> = b"Combined NFT of ";
        let new_description_string: String = string::utf8(new_description_vector);
        string::append(&mut new_description_string, nft1.name);
        let vec2: vector<u8> = b" and ";
        string::append(&mut new_description_string, string::utf8(vec2));
        string::append(&mut new_description_string, nft2.name);
    ...
    */
    public fun withdraw_sales(
        ...
        let withdrawAmount=balance::withdraw_all(&mut minter_cap.sales);
        
        event::emit(SalesWithdrawn{
            amount: balance::value(&withdrawAmount)
        });
        coin::from_balance(withdrawAmount,ctx)
    }
